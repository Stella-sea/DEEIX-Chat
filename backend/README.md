...
管理员可在 MCP Server 的请求头中配置签名用户上下文头：把某个请求头的值填为占位符 `${DEEIX_SIGNED_USER_CONTEXT}`，每次用户工具调用时该头会被替换为 HMAC-SHA256 签名的 token（payload 含 `user_id`、`conversation_id`、`request_id`、`aud`、`jti` 与过期时间，签名密钥来自独立的 `MCP_USER_CONTEXT_SECRET`，默认有效期 5 分钟）。其中 `aud` 绑定目标服务地址，防止 token 被重放到其他 MCP 服务端；`jti` 每次调用唯一，服务端可用于重放防护与幂等。MCP 服务端或外部网关可用同一 MCP 密钥校验，按用户隔离单租户 MCP 工具。工具同步时会忽略该占位符，不会把占位符原文发送给 MCP。启用了占位符但未配置签名密钥时，工具调用会失败并不会发送请求。未配置占位符的服务端不会收到任何额外请求头。
